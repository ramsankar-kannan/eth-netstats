	...
	if( !_.isUndefined(block) && !_.isUndefined(block.number) && !_.isUndefined(block.uncles) && !_.isUndefined(block.transactions) && !_.isUndefined(block.difficulty) && block.number > 0 )
	{
			...
			if( propIndex === -1 )
			{
				...
				else
				{
					...
					if( prevBlock )
					{
						...
						console.log("===========>set block time " + block.time)
					}
					else
					{
						block.time = 0;
		...
		else
		{
			...
			if( prevBlock )
			{
				...
				//block.time = 0;
				//console.log('reset block.time arrived to 0 ' + block.number + ' ' + this.bestBlock().height);
			...
			{
				console.log("!!!!!! No prevblock " + block.number)
				block.time = 0;
			}
...
History.prototype.getCharts = function()
{
	if(this._callback !== null)
	{
		...
		//console.log('map chartHistory blocktime:', _.map( chartHistory, 'blocktime'));
